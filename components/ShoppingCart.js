...
class ShoppingCart extends Component {
  ...
  handleCoupon = () => {
    fetch(`https://app.shoppingrunway.mx/api/v1/coupon/${this.state.coupon}`)
    .then((response) => response.json())
    .then((responseJson) => {
      if (responseJson.error) {
        this.setState({couponMessage: responseJson.message});
        return;
      }
      if (!responseJson.coupon.isValid) {
        this.setState({couponMessage: 'Your coupon is not valid'});
        return;
      }
      if (responseJson.coupon.isValid && responseJson.coupon.percentaje != 0) {
        this.setState({couponMessage: `You get ${responseJson.coupon.percentaje}% discount off your purchase`, couponPercent: responseJson.coupon.percentaje});
        return;
      }
    })
    .catch((error) => {
  ...
  handleCheckoutClick = async () => {
    ...
    if (user !== null) {
      ...
      const couponPercent = this.state.couponPercent;
      this.props.navigation.navigate("Page1", { total, products, couponPercent });
    } else {
      ...
      <>
        <Container>
          ...
          <Content style={{ flex: 1 }}>
            {loading ? (
              <Spinner style={{ marginTop: 80 }} size={0} color="#555" />
            ...
            {cartProducts.length > 0 && (
              <>
              {(Total >= 899) ? (<View style={{marginVertical: 18, alignItems: 'center'}}><Text style={{ fontSize: 17, color: "#444" }}>Shipping fee is waved for orders over $899</Text></View>) : null }
              <View style={{flexDirection: "row", justifyContent: "space-between", marginVertical: 18, marginHorizontal:14}}>
                <TextInput
                  ...
                  padding: 8,
                  borderRadius: 8,
                  backgroundColor: "#fff", flex:3,marginRight: 8}}
                  onChangeText={coupon => this.setState({ coupon })}
                  value={this.state.coupon}/>
                <Button onPress={this.handleCoupon} style={[styles.payBtn,{flex:1}]}>
                  <Text style={[{ fontSize: 17 }, styles.white]}>Enviar</Text>
                </Button>

              </View>
              {(this.state.couponMessage) ? (<View style={{marginBottom: 18, alignItems: 'center'}}><Text style={{ fontSize: 17, color: "#444" }}>{this.state.couponMessage}</Text></View>) : null }
